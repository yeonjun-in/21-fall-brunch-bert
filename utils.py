...
def get_name_of_experiment_path(experiment_dir, experiment_description, args):
    experiment_path = os.path.join(experiment_dir, (experiment_description + "_" + config2string(args)))
    idx = _get_experiment_index(experiment_path)
    experiment_path = experiment_path + " " + str(idx)
    return experiment_path
...
def config2string(args):
    ...
    for name, val in zip(args_names, args_vals):
        if val == False:
            continue
        if name in ['min_uc', 'min_sc', 'bert_add_author', 'bert_add_magazine', 'bert_emb_aggr', 'bert_aug_list']:
            st_ = "{}={} | ".format(name, val)
            st += st_
...
def _get_experiment_index(experiment_path):
    idx = 0
    while os.path.exists(experiment_path + " " + str(idx)):
        idx += 1
    return idx
